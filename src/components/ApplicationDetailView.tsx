...
import { JobApplication, UserProfile, ApplicationStatus, RecruiterCandidate } from '../types';
...
export const ApplicationDetailView: React.FC<ApplicationDetailViewProps> = ({
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-5">
      {/* Atlassian Clean Minimalism Detail Header */}
      <div className="bg-white border border-[#DFE1E6] rounded-[4px] p-5 shadow-[0_1px_3px_rgba(9,30,66,0.08)] space-y-4">
        ...
        {/* Job Title & Meta Info with Quick Tip Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 pt-2 border-t border-[#DFE1E6] items-start">
          ...
          {/* Quick Tip Card in Atlassian Blue */}
          <div className="p-3.5 bg-[#0052CC] text-white rounded-[4px] shadow-sm flex flex-col justify-between space-y-1.5">
            <div className="flex items-center justify-between text-[11px] font-semibold opacity-90 uppercase tracking-wider">
              <span>QUICK TIP: LATEX INVARIANT</span>
              <Sparkles className="w-3 h-3 text-[#FFE380]" />
            </div>
            <p className="text-xs leading-relaxed opacity-95">
              Only bullet points and role highlights are edited. LaTeX preamble, document geometry, and macros remain 100% intact.
            </p>
          </div>
        </div>

        {/* Quick Action Launchers */}
        <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-[#DFE1E6]">
          <button
            ...
            className="px-3.5 py-2 bg-[#0052CC] hover:bg-[#0747A6] disabled:opacity-50 text-white text-xs font-bold rounded-[3px] shadow-sm transition-colors flex items-center space-x-1.5 cursor-pointer"
          >
            {isTailoring ? (
              <>
        ...
        {/* Atlassian Tab Strip for Detail View */}
        <div className="flex space-x-2 overflow-x-auto pt-2 border-t border-[#DFE1E6] scrollbar-none">
          ...
          >
            ...
            {hasCoverLetter && (
              <span className="w-2 h-2 rounded-full bg-[#008DA6]" />
            )}
          </button>
